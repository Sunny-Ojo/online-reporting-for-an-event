...
if (isset($_POST["submit"])) {
    ...
    try {
        ...
        if (!empty($_FILES["pdf"])) {
            ...
            $updateReport = $conn->prepare("UPDATE $day SET coordinatorsName=?,    departmentName=?,    timeOpen=?,    timeClose=?,    numberOfTeens=?,    numberOfTeachers=?,    reportFortheDay=?,challenges=?, possibleSolutions=?, relevantInformation=?, pdf=? WHERE id=?");
            $save = $updateReport->execute([$name, $department, $timeopen, $timeclose, $numberofteens, $numberofteachers, $reportfortheday, $challenges, $possible_solutions, $relevant_information, $fileToDb, $_POST["id"]]);
            $_SESSION["success"] = 'Report has been successfully updated, thank you for your time';
            //redirect the user where you want them to go after submitting a report
            header('Location: edit.php');
        } else {
            $updateReport = $conn->prepare("UPDATE $day SET coordinatorsName=?,    departmentName=?,    timeOpen=?,    timeClose=?,    numberOfTeens=?,    numberOfTeachers=?,    reportFortheDay=?,challenges=?, possibleSolutions=?, relevantInformation=? WHERE id=?");
            $save = $updateReport->execute([$name, $department, $timeopen, $timeclose, $numberofteens, $numberofteachers, $reportfortheday, $challenges, $possible_solutions, $relevant_information, $_POST["id"]]);
            $_SESSION["success"] = 'Report has been successfully updated, thank you for your time';
            //redirect the user where you want them to go after submitting a report
            header('Location: edit.php');
        }
